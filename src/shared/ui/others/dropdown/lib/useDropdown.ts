...
export const useDropdown = ({
  ...
  const [internal, setInternal] = useState<Array<string | number>>(toArray(defaultValue));
  ...
  const toggleOption = (opt: Option) => {
    ...
    if (multiple) {
      next = exists ? selected.filter((v) => v !== opt.value) : [...selected, opt.value];
    } else {
      next = exists ? [] : [opt.value];
    }
